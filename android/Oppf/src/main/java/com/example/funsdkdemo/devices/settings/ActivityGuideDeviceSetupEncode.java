...
public class ActivityGuideDeviceSetupEncode extends ActivityDemo
    ...
    int GetResolutionSize(int nResIndex) {
        if (nResIndex >= 0 && nResIndex <= SDK_CAPTURE_SIZE_t.SDK_CAPTURE_SIZE_4K_N) {
            return s_fps[nResIndex];
        }
        return s_fps[0];
    }

    long GetResMark(long nLastAbility, int nRate, long nSupportResMark) {
        long nRetMark = 0;
        int nGetRes = 0;
        for (int i = 0; i <= SDK_CAPTURE_SIZE_t.SDK_CAPTURE_SIZE_4K_N; ++i) {
            if (0 != (nSupportResMark & (((long) 0x1) << i))) {
    ...
    public int N_RESOLUTION_COUNT = SDK_CAPTURE_SIZE_t.SDK_CAPTURE_SIZE_4K_N;

    long GetMainResMark() {
        long nRetMark = 0;
        nRetMark = mImageSizePerChannel;
        if (nRetMark == 0) {
            nRetMark = mResolutionMask;
        }
    ...
    String GetResFromIndex(int index) {
        if (index >= 0 && index <= SDK_CAPTURE_SIZE_t.SDK_CAPTURE_SIZE_4K_N) {
...
//		System.out.println("TTT-----mainframe = " + simplifyEncode.extraFormat.video.FPS);
    ...
    public void onItemSelected(AdapterView<?> av, View view, int position, long id) {
        ...
        int avId = av.getId();
        if (avId == R.id.sp_main_resolution || avId == R.id.sp_main_frame) {
            ...
            if (_nLastRes != nMyRes | nMyRate != _nLastRate) {
                _nLastRes = nMyRes;
                _nLastRate = nMyRate;
                long nLastAbility = GetLastAbility(mMaxEncodePowerPerChannel, nMyRes, nMyRate);
                ...
                long nRetMark = GetResMark(nLastAbility, nSubRate, nSupportResMark);
        ...
        } else if (avId == R.id.sp_sub_resolution) {
            int nMyRes = GetIntValue(R.id.sp_main_resolution);
            int nMyRate = GetIntValue(R.id.sp_main_frame);
            long nLastAbility = GetLastAbility(encode.MaxEncodePowerPerChannel, nMyRes, nMyRate);
            int nSupportResMark = GetSubResMark(nMyRes);
            int nSubRes = GetIntValue(R.id.sp_sub_resolution);
            int nSubRate = GetIntValue(R.id.sp_sub_frame);

            long nRetMark = GetResMark(nLastAbility, nSubRate, nSupportResMark);
            if (0 != (nRetMark & (((long) 0x1) << nSubRes))) {
                return;
            }
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.backBtnInTopLayout) {
            finish();
            // 返回
        } else if (id == R.id.ck_main_voice || id == R.id.ck_sub_video || id == R.id.ck_sub_voice) {
    ...
    long GetLastAbility(long nMaxPower, int nResIndex, int nRate) {
        return nMaxPower - GetResolutionSize(nResIndex) * nRate;
    }

    void UpdateResSpanner(int id, long nMark) {
        int nCount = 0;
        for (int i = 0; i <= SDK_CAPTURE_SIZE_t.SDK_CAPTURE_SIZE_4K_N; ++i) {
            if (0 != (nMark & (((long) 0x1) << i))) {
                nCount++;
            }
        }
    ...
    int GetMaxRate(long nLastAbility, int nResIndex) {
    ...
    public boolean onTouch(View v, MotionEvent me) {
        ...
        if (me.getAction() == MotionEvent.ACTION_DOWN) {
            int id = v.getId();
            if (id == R.id.sp_main_resolution) {
                long nLastAbility = this.GetLastAbility(mMaxEncodePowerPerChannel, _nMinSubRes, 1);
                /*
                 * 以前结构体的配置方法
                 */
                ...
                // 1);
                long nSupportResMark = this.GetMainResMark();
                int nRate = this.GetIntValue(R.id.sp_main_frame);
                long nRetMark = this.GetResMark(nLastAbility, nRate, nSupportResMark);
                Log.d("TTT", "TTT　nRetMark：" + nRetMark);
                UpdateResSpanner(R.id.sp_main_resolution, nRetMark);
            } else if (id == R.id.sp_main_frame) {
                long nLastAbility = GetLastAbility(mMaxEncodePowerPerChannel, _nMinSubRes, 1);
            ...
            } else if (id == R.id.sp_sub_resolution) {
