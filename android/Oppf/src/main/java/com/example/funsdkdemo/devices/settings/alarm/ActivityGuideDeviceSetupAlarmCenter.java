...
public class ActivityGuideDeviceSetupAlarmCenter extends ActivityDemo implements
	...
	public void onClick(View v) {
		...
		if (id == R.id.backBtnInTopLayout) {// 返回/退出
			finish();
		} else if (id == R.id.cb_device_log_up) {// 日志上报

		} else if (id == R.id.cb_device_warn_up) {// 警报上报

		} else if (id == R.id.cb_device_use) {// 设置是否启用
			tryDeviceUseSet();
		} else if (id == R.id.et_address) {// 设置服务器地址

		} else if (id == R.id.et_port) {// 设置端口号
