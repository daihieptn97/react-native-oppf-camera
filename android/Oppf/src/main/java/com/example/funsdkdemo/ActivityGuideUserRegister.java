...
public class ActivityGuideUserRegister extends ActivityDemo implements OnClickListener, OnCheckedChangeListener, OnFunRegisterListener{
	...
	public void onClick(View v) {
		...
		if (id == R.id.backBtnInTopLayout) {// 返回/退出
			finish();
		} else if (id == R.id.userRegisterBtn) {// 快速注册
			tryToRegister();
		} else if (id == R.id.btnGetVerifyCode) {// 获取验证码
			tryGetVerifyCode();
		} else if (id == R.id.btnUserNameRepeat) {//check username repeat
			checkUsername();
		}
	}
	...
	public void onCheckedChanged(RadioGroup radioGroup, int id) {
		if (id == R.id.radioBtnRegisterByEmail) {
			byEmail = true;
			showRegisterLayout(byEmail);
		} else if (id == R.id.radioBtnRegisterByCellphone) {
			byEmail = false;
			showRegisterLayout(byEmail);
		}
	}
