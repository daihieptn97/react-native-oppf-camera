...
import com.hunonic.funsdkdemo.ListAdapterDeviceComComands.ComCommand;
...
public class ActivityGuideDeviceTransCom extends ActivityDemo implements OnClickListener, OnFunDeviceOptListener, OnFunDeviceSerialListener {
	...
	public void onClick(View v) {
		...
		if (id == R.id.backBtnInTopLayout) {// 返回/退出
			finish();
		} else if (id == R.id.btnSend) {// 4. 测试串口命令  - 成功打开串口之后才能测试写串口命令
			sendComData();
		} else if (id == R.id.btnTest1) {// 4. 测试串口命令  - 成功打开串口之后才能测试写串口命令
			sendTestQS1();
		} else if (id == R.id.btnTest2) {// 4. 测试串口命令  - 成功打开串口之后才能测试写串口命令
			sendTestQS2();
		} else if (id == R.id.btnTest3) {// 4. 测试串口命令  - 成功打开串口之后才能测试写串口命令
			sendComTest3();
		}
	}
